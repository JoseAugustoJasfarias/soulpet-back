...
const agendamentoSchema = Joi.object({
  ...
  function formatErrorMessage(error) {
    const messages = error.details.map(detail => {
      ...
      message = message.replace('must be a valid date', ' deve ser uma data futura , data válida entre aspas no formato ANO/MES/DIA.');
...
router.post('/agendamentos', async (req, res) => {
    ...
    if (!Array.isArray(agendamento) || agendamento.length === 0) {
      return res.status(400).json({
        message: 'A propriedade "agendamento" deve ser um array com pelo menos um objeto.'
      });
    }
  
    const { error } = Joi.array().items(agendamentoSchema).validate(agendamento, { abortEarly: false });
    ...
    try {
      for (const item of agendamento) {
        ...
        if (!pet) {
          return res.status(404).json({ message: `Pet com ID ${petId} não encontrado.` });
        }
  
        if (!servico) {
          return res.status(404).json({ message: `Serviço com ID ${servicoId} não encontrado.` });
        }
        ...
        res.status(201).json({ message: 'Agendamento criado com sucesso!', agendamento: agendamentoObj });
      }
    } 
        catch (err) {
            ...
            if (err.name === 'SequelizeForeignKeyConstraintError') {
              return res.status(400).json({ message: 'Chave estrangeira não encontrada' });
            }
          
            // Verifica se o erro é de constraint de chave única
            if (err.name === 'SequelizeUniqueConstraintError') {
              return res.status(400).json({ message: 'Já existe um pedido com o mesmo número.' });
            }
  ...
  router.put('/agendamentos/:id', async (req, res) => {
    ...
    try {
      for (const item of agendamento) {
        ...
        if (rowsUpdated === 0) {
          return res.status(404).json({ message: `Agendamento com ID ${id} não encontrado.` });
        }
  
        const updatedAgendamento = await Agendamento.findOne({ where: { servicoId } });
  
        res.json({ message: 'Agendamento atualizado com sucesso!', agendamento: updatedAgendamento });
      }
    } catch (err) {
      // Verifica se o erro é de validação do Joi
      if (err.name === 'ValidationError') {
